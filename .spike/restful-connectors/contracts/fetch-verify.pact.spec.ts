...
const responseSpecification = eachLike({
  ...
  status: term({
    generate: 'PENDING_APPROVAL',
    matcher: 'PENDING_APPROVAL|COMPLETE|DECLINED'
  }),
...
const responseCode200 = {
  state: 'has some verify requests',
  uponReceiving: 'a get for verify requests',
  withRequest: {
    method: 'GET',
    path: endpoint,
    query: {
...
let provider;
describe('verify fetch details service', () => {
  before(() => {
    provider = new Pact({
  ...
  describe('fetches verify requests', () => {
    ...
    beforeEach(async function () {
      ...
      requests = await service.fetch('VERIFY');
      request = requests[0];
    });
    ...
    describe('with a 200 server response', () => {
      ...
      context('a single request contains a', () => {
        ...
        it('type', () => {
          request.type.should.equal('VERIFY');
        });
        ...
        it('status', () => {
          request.status.should.equal('PENDING_APPROVAL');
        });
